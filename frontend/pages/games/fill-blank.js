...
export default function FillBlankPage() {
  ...
  const handleAnswer = (index) => {
    ...
    if (index === questions[currentQ].correct) {
      ...
      toast.error(`Answer: ${questions[currentQ].options[questions[currentQ].correct]}`)
      setStreak(0)
    }
    setTimeout(() => {
      ...
      <div className="min-h-screen py-8 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <motion.div key={currentQ} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-8 text-center">
            <h2 className="text-2xl font-display font-semibold mb-8">{questions[currentQ].template}</h2>
