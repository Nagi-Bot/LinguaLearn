...
async function seed() {
  try {
    const uri = process.env.MONGODB_URI || 'mongodb+srv://haanmoorad_db_user:S50lKXAjP6BiCCxJ@cluster0.r5p5u0u.mongodb.net/lingualearn?retryWrites=true&w=majority&appName=Cluster0'
    ...
    for (const data of users) {
      ...
      await user.save()
      console.log(`Created user: ${data.name} (Level ${data.level}, ${data.xp} XP)`)
    }

    console.log('\nSeed complete! 13 users created.')
    process.exit(0)
  } catch (err) {
